...
GitUser=Masitah1237
BURIQ () {
    curl -sS https://raw.githubusercontent.com/${GitUser}/percubaan/main/authipvps > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/${GitUser}/percubaan/main/authipvps | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/${GitUser}/percubaan/main/authipvps | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
